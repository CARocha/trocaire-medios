...
from crisis_alimentaria.models import *
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for credito in AccesoCredito.objects.all():
            hombre = credito.hombre.all().values_list('id', flat=True)
            mujer = credito.mujer.all().values_list('id', flat=True)
            #chequear si algun valor de foo (no tiene y no aplica) esta en en hombre o mujer             
            if _list_in_list(foo, hombre) and _list_in_list(foo, mujer):
                credito.encuesta.credito = 2
                print '%s no tiene credito' % credito.encuesta.id
            else:
                credito.encuesta.credito = 1
                print '%s tiene credito' % credito.encuesta.id
                            
            credito.encuesta.save()
