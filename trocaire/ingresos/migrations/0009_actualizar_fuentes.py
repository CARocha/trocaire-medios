...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for fuente in  orm.PrincipalesFuentes.objects.all():
            fuente.fuentes_ap = fuente.fuente.filter(id__in = [1,2,3,4,5]).count()
            fuente.fuentes_no_ap = fuente.fuente.exclude(id__in = [1,2,3,4,5]).count()
            fuente.save()
